...
def transform_string(s):
    def transformer(st):
        ind = 0
        if st[ind].isupper(): st.replace(st[ind], st[ind].lower())
        else: st.replace(st[ind], st[ind].upper())
        ind += 1
        return st
    colon = s.find(":")
    before_s = s[:colon]
...
def transform_string(s):
    colon = s.find(":")
    bef_s = "".join([x.upper() if x.islower() else x.lower() for x in s[:colon]])
    aft_s = "".join([x.upper() if x.islower() else x.lower() for x in s[colon+1:]])
